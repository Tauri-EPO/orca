...
A `failed` start that left its own agent terminal behind also returns `recoveryNote` and `recoveryCommands`, unless the receipt already carries host-supplied commands such as `nextCommands`, which take precedence and are followed as printed. When Orca does add them, every command they name routes through the Dispatch: `orca orchestration worker-release --dispatch <dispatch_id> --json` on this server, or `orca orchestration worker-show --dispatch <dispatch_id> --json` when the worker ran on a connected server that owns the handle. Release re-checks ownership on the host that holds the terminal, preserves inspectable output first, and retains anything it cannot prove that Dispatch owns, so it is safe to run on a receipt whose `residualResources` also list setup terminals, configured tabs or a reused terminal. An `outcome_unknown` start never gets reclaim guidance, because it may have succeeded: re-issue with `--retry-request` and act on the receipt the retry returns. When no command is offered, nothing in the receipt proved reclaimable ownership; use `worker-show` before touching any handle.
...
Do not release a worker because of a timeout, TUI idle state, heartbeat, status, question, escalation, or rejected/stale `worker_done`. If release returns `release_pending` or `release_unknown`, do not substitute `terminal close`; follow the exact recovery action in the receipt. A replayed Delivery may repeat `worker-release` safely. `terminal close` is legitimate for exactly one orchestration case, and proving it takes two receipts, not one: the originating `failed` `worker-start` receipt must name that exact terminal in its own `residualResources`, showing this start created it before Orca could take authority over it, and `worker-release` on that same Dispatch must then report `no_owned_resource`. A release receipt alone proves nothing — a Dispatch created by `orchestration dispatch --inject` reports the same `retained` / `no_owned_resource` for a terminal it never owned, and that terminal takes no process action at all. Every other retained reason stays excluded.
